...
@connect(({organization, loading}) => ({
  organization,
  loading: loading.effects['organization/fetch_permissions_role_list_action'],
}))
@injectIntl
export default class RoleList extends Component {
  ...
  componentDidMount(){
    const {organization:{permissions_role_params,permissions_role_list}} = this.props;
  ...
  loadPermissionsRoles=(params,isSearch)=>{
    this.props.dispatch({
      type:'organization/fetch_permissions_role_list_action',
      payload:params,
      isSearch:isSearch,
      callBack:(value)=>{
        //console.log(value)
  ...
  handleInfiniteOnLoad=()=>{
    const {organization:{permissions_role_params}} = this.props;
    const isSearch = false;
  ...
  render(){
    const {organization:{permissions_role_list,permissionsRoleHasMore},loading,intl:{formatMessage}  } = this.props;
    const {modalVisible,onCancelModal,handleRoleSubmit} = this.props;
    return(
      <Modal
        visible={modalVisible}
        title={formatMessage(basicMessages.departRoles)}
        //destroyOnClose={true}
        width={400}
