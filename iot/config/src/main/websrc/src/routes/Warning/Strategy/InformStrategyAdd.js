...
import {Table, Button, Icon, Badge, Form, Input, Card,} from 'antd';
...
@connect(({warning, warningStrategy, loading}) => ({
  warning,
  warningStrategy,
  loading: loading.effects['warningStrategy/fetch_addWarningStrategyList_action'],
}))
@injectIntl
@Form.create()
  ...
  handleSubmit = (e) => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.props.dispatch({
          type: 'warningStrategy/fetch_addWarningStrategyList_action',
          payload: value,
        });
      }
    });
                  ...
                  getFieldDecorator('accessKey', {
                    ...
                    <div onClick={this.changeModalVisible} className={styles.ele_input_addStype} style={{height: 35}}>
                      <div style={{padding: '0 8px', height: 35, lineHeight: '35px'}}>
                        <span>{checkedValue && checkedValue.serviceName}</span>
                      </div>
                      <Input defaultValue={checkedValue && checkedValue.serviceName} style={{display: 'none'}}/>
