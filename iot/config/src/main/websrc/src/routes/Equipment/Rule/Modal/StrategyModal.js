...
export default class PlugModal extends Component {
  ...
  componentDidMount() {
    ...
    if(componentPullList.length===0||componentPullList_params.status!==status||componentPullList_params.type!==type){
  ...
  handleInfiniteOnLoad = () => {
    const {equipment: {componentPullList_params}} = this.props;
  ...
  render() {
    const {modalVisible, onCancelModal, equipment: {componentPullList,componentPullMore}, loading, handleCheckValue,modalCheckedActionValue} = this.props;
    return (
      <Modal
      ...
      >
        <div style={{padding: 24}}>
          ...
          />
          <Radio.Group
            value={this.state.checkValue&&this.state.checkValue.id?JSON.stringify(this.state.checkValue):JSON.stringify(modalCheckedActionValue)}
