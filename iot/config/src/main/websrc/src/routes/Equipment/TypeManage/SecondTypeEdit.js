...
@connect(({equipmentTypeManage, loading}) => ({
  equipmentTypeManage,
  loading: loading.effects['equipmentTypeManage/fetch_editSecondType_action'],
}))
@injectIntl
@Form.create()
export default class SecondTypeAdd extends Component {
  ...
  componentDidMount(){
    ...
    this.setState({
      checkedFirstType:{name:item.deviceTypeName,id:item.deviceTypeId},
    })
    this.props.form.setFieldsValue({deviceTypeId:item.deviceTypeId});
    this.props.dispatch({
      type:'equipmentTypeManage/fetch_getSecondTypeItem_action',
      payload:{
        id:item.id,
      },
    })
  ...
  handleSubmit = (e) => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        this.props.dispatch({
          type:'equipmentTypeManage/fetch_editSecondType_action',
          payload:{
            id:item.id,
            ...values,
          },
  ...
  render() {
    const {loading,intl:{formatMessage},equipmentTypeManage:{getSecondTypeItem}} = this.props;
              ...
              >
                {
                  getFieldDecorator('name', {
                    ...
                    initialValue:getSecondTypeItem&&getSecondTypeItem.name
                  })(
