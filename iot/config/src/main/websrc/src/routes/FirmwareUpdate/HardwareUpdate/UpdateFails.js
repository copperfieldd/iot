...
@connect(({hardwareUpdate, loading}) => ({
  hardwareUpdate,
  loading: loading.effects['hardwareUpdate/fetch_upgradeItem_action'],
}))
@injectIntl
@Form.create()
export default class UpdateFails extends Component {
  ...
  componentDidMount(){
    const {hardwareUpdate:{upgradePackageDetails_params},match:{params:{id}}} = this.props;
    let params = {
      upgradeId:id,
      start:0,
  ...
  loadList=(params)=>{
    this.props.dispatch({
      type:'hardwareUpdate/fetch_upgradeItem_action',
      payload:{
        ...params,
        state:2,
      }
  ...
  onChangeTablePage=(pagination)=>{
    const {hardwareUpdate:{upgradePackageDetails_params}} = this.props;
    const params = {
  ...
  render() {
    const {hardwareUpdate:{upgradePackageDetails_params,upgradePackageDetails},intl:{formatMessage},loading} = this.props;
    ...
    let packageName = {title: formatMessage(messages.firm_package_name), dataIndex: 'upgradePackageName', key: 'upgradePackageName', className:'table_row_styles',render:(text,record)=>(
        <span>{record.upgradePackageName}</span>
      )};
      ...
      <PageHeaderLayout>
        ...
        <div className='TxTCenter' style={{marginTop: 20}}>
          <Button className='mrgLf20'
                  onClick={() => {
                    this.props.dispatch(routerRedux.push(`/firmwareUpdate/hardwareUpdate`))
                  }}
