...
@connect(({permissionsGrade, loading}) => ({
  loading: loading.effects['permissionsGrade/fetch_getNoInventoryList_action'],
}))
@Form.create()
export default class TenantGradeEdit extends Component {
  constructor() {
    super();
    this.state = {
      expandedKeys:[],
  ...
  render() {
    const {expandedKeys, visible,apiModalVisible} = this.state;
    ...
    return (
      <PageHeaderLayout>
        ...
        >
          <Card
            title={<span style={{color:'#3f89e1'}}>修改等级</span>}
          >
          <div className='mrgTB30' style={{width:500}}>
            <Form>
              ...
              >
                {
                  getFieldDecorator('apiList', {
                    //initialValue: details&&details.name
                  })(
                    <div onClick={this.openApiModal} className={styles.ele_input_addStype}>
                      {checkedApiList&&checkedApiList?checkedApiList.map((item,index)=>{
                        return(
