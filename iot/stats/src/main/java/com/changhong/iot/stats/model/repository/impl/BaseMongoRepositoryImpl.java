...
import com.changhong.iot.stats.model.repository.BaseMongoRepository;
...
public abstract class BaseMongoRepositoryImpl<T> implements BaseMongoRepository<T> {
    ...
    @Override
    public PageModel<T> pageAll(Criteria criteria, int start, int count) {
        Query query = Query.query(criteria);
        query.with(new Sort(Direction.DESC, FIELD_CREATE_TIME));
    ...
    public PageModel<HashMap> aggregate(List<AggregationOperation> aggregationOperations, int start, int count,String collection) {
        return this.aggregate(aggregationOperations, collection, start, count);
    }
...
//    }
    ...
    @Override
    public PageModel<HashMap> aggregate(List<AggregationOperation> aggregationOperations,String collection, int start, int count) {
        return this.aggregate(aggregationOperations, collection, start, count, false);
    }

    @Override
    public PageModel<HashMap> aggregate(List<AggregationOperation> aggregationOperations, String collection, int start, int count, boolean globalCount) {
        ...
        if (0 <= start && 0 < count) {
            ...
            AggregationResults<HashMap> aggregate = mgt.aggregate(aggregation,collection, HashMap.class);
            ...
            aggregationOperations.add(Aggregation.sort(new Sort(Sort.Direction.DESC, FIELD_CREATE_TIME)));
            aggregationOperations.add(Aggregation.skip((long) start));
            aggregationOperations.add(Aggregation.limit(count));
        }
        Aggregation aggregation = Aggregation.newAggregation( aggregationOperations);
        List<HashMap> list = mgt.aggregate(aggregation,collection, HashMap.class).getMappedResults();
